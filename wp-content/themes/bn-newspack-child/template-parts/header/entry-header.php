...
if ( $sharing_enabled ) :
	...
	<div class="entry-subhead">
		...
		<?php endif; ?>
		<?php
		// Display reading time for posts and articles
		if ( ! is_page() && function_exists( 'bn_get_reading_time' ) ) {
			$reading_time = bn_get_reading_time();
			if ( $reading_time > 0 ) {
				?>
				<div class="bn-reading-time">
					<span class="reading-time-text">
						<?php
						echo esc_html(
							sprintf(
								/* translators: %s: reading time in minutes */
								_n( 'Reading Time: %s minute', 'Reading Time: %s minutes', $reading_time, 'bn-newspack-child' ),
